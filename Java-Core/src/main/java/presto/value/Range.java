...
public abstract class Range<T extends IValue> extends BaseValue implements ICollection<T>, IRange<T> {
	...
	@SuppressWarnings("unchecked")
	public T getItem(Context context, IValue index) throws PrestoError {
	...
	public Range<T> slice(Integer fi, Integer li) throws PrestoError {
		long size = length();
