...
public class Date extends BaseValue implements Comparable<Date>
{
    ...
    public IValue Subtract(Context context, IValue value) throws PrestoError
    {
        if (value instanceof Date)
        {
			...
			org.joda.time.Period result = new org.joda.time.Period( this.value.getYear() - other.getYear(),
					this.value.getMonthOfYear() - other.getMonthOfYear(),
					0,
					this.value.getDayOfMonth() - other.getDayOfMonth(),
					0, 0, 0, 0);
    ...
    public int CompareTo(Context context, IValue value) throws PrestoError
    {
        if (value instanceof Date)
            ...
            throw new SyntaxError("Illegal comparison: Date - " + value.getClass().getSimpleName());
