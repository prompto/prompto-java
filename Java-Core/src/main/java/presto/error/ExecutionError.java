...
public abstract class ExecutionError extends PrestoError {
	...
	public IValue interpret(Context context, String errorName) throws PrestoError {
		IExpression exp = this.getExpression(context);
		if(exp==null) {
			ArgumentAssignmentList args = new ArgumentAssignmentList();
			args.add(new ArgumentAssignment(new UnresolvedArgument("name"), new TextLiteral(this.getClass().getSimpleName())));
			args.add(new ArgumentAssignment(new UnresolvedArgument("text"), new TextLiteral(this.getMessage())));
