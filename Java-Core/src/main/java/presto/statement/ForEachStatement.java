...
public class ForEachStatement extends BaseStatement {
	...
	private IValue evaluateItemIterator(IType elemType, Context context) throws PrestoError {
		if (v2 == null)
	...
	private IValue evaluateItemIteratorNoIndex(IType elemType, Context context) throws PrestoError {
	...
	private IValue evaluateItemIteratorWithIndex(IType elemType, Context context) throws PrestoError {
		IValue src = source.interpret(context);
		Iterator<IValue> iterator = getIterator(context, src);
		long index = 0L;
		while (iterator.hasNext()) {
