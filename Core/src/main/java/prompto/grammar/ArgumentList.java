...
public class ArgumentList extends LinkedList<Argument> {
	...
	public ArgumentList makeArguments(Context context, IMethodDeclaration declaration) {
		ArgumentList local = new ArgumentList(this);
		ArgumentList arguments = new ArgumentList();
		for(int i=0;i<declaration.getParameters().size();i++) {
		    IParameter parameter = declaration.getParameters().get(i);
		    Argument argument = null;
	        int index = local.findIndex(parameter.getId());
		    if(index<0 && i==0 && this.size()>0 && this.get(0).getParameter()==null)
		        index = 0;
		    if(index>=0) {
	            argument = local.get(index);
	            local.remove(index);
	        }
	        if(argument==null) {
	            if (parameter.getDefaultExpression() != null)
	                arguments.add(new Argument(parameter, parameter.getDefaultExpression()));
	            else
	            ...
	            arguments.add(new Argument(parameter, expression));
	        }
	    }
	    if(local.size() > 0) {
	    	Argument argument = local.get(0);
